...
× need to give better examples about why you picked clojure over other languages.  answer:  "why clojure?"  The decision factor wasn't really presented.
  -> That's not the question this talk seeks to answer. The idea is to present what worked for us with Clojure/Script. Perhaps a different title?
✓ Change slide titles to better reflect the goal of the talk.
  ✓ talk about our experience
  ✓ not provide retroactive justification for decisions
  ✓ introduction to tech & techniques used
  - some conclusions we've drawn
✓ Your examples should continually feed into how it helped birdwave development
  ✓ "can i import a gigantic data file" was much better at doing this
...
- Use making the app responsive in the Conclusions section as an example of making full-stack changes easily
  - one language
  - swap components live
  - ease of debugging by inspecting state (because view is idempotent fn of state)
